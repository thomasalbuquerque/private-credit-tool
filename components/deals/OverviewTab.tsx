...
function RiskRow({ label, status, isLast }: { label: string; status: RiskStatus; isLast: boolean }) {
  return (
    <div className={cn('flex items-center justify-between py-3', !isLast && 'border-b border-slate-700')}>
    ...
    <div className='grid grid-cols-1 gap-6 lg:grid-cols-3'>
      ...
      <div className='space-y-6 lg:col-span-2'>
        ...
        <Card title='Risk Assessment'>
          ...
          <div className='mt-4'>
            {RISK_ROWS.map((row, i) => (
              <RiskRow key={row.key} label={row.label} status={deal.riskBreakdown[row.key]} isLast={i === RISK_ROWS.length - 1} />
            ))}
          </div>
        </Card>
      </div>
      ...
      <div className='space-y-6 lg:col-span-1'>
        <Card title='Deal Timeline'>
          {deal.timeline.length === 0 ? (
            <p className='text-sm text-slate-500'>No timeline steps yet.</p>
          ) : (
            <div>
              {deal.timeline.map((step, i) => {
                ...
                return (
                  <div key={`${step.label}-${i}`} className='relative flex gap-3'>
                    {!isLast && <span className='absolute left-1.5 top-4 h-full w-px bg-slate-500' />}
                    <span
                      className={cn(
                        'relative mt-1 h-3.5 w-3.5 shrink-0 rounded-full',
                        step.completed ? 'bg-indigo-500' : 'border-2 border-slate-400 bg-slate-800',
                      )}
                    />
                    <div className='min-w-0 pb-5'>
                      <p className={cn('text-sm leading-snug', step.completed ? 'font-medium text-slate-100' : 'text-slate-400')}>{step.label}</p>
              ...
              {deal.recentActivity.slice(0, 3).map((item, i) => (
                <li key={`${item.date}-${i}`} className='flex gap-2.5'>
                  ...
                  <div className='min-w-0'>
                    <p className='text-sm leading-snug text-slate-300'>{item.action}</p>
                    <p className='mt-0.5 text-xs text-slate-500'>{relativeDate(item.date)}</p>
                  </div>
                </li>
              ))}
            </ul>
