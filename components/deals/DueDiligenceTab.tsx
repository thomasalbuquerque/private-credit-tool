...
import { cn, findingStatusBadgeVariant, getInitials, inputClass, riskLevelAccent, riskLevelBadgeVariant, workstreamStatusBadgeVariant } from '@/lib/utils';
...
function WorkstreamIcon({ status }: { status: Workstream['status'] }) {
  ...
  if (status === 'In Progress') return <Clock className='h-4 w-4 text-blue-400' />;
  return <Circle className='h-4 w-4 text-slate-600' />;
}

function FindingCard({ finding }: { finding: Finding }) {
  return (
    <div className={cn('rounded-lg border border-slate-600 border-l-4 bg-slate-800 p-4', riskLevelAccent(finding.riskLevel))}>
      ...
      <div className='mt-3 flex items-center gap-2'>
        ...
        <span className='text-xs font-medium text-slate-200'>{finding.owner}</span>
      </div>
    </div>
  );
}
...
export default function DueDiligenceTab({ dealId }: DueDiligenceTabProps) {
  ...
  const completeCount = useMemo(() => (diligence ? diligence.workstreams.filter((w) => w.status === 'Complete').length : 0), [diligence]);
  ...
  if (!diligence) {
    return (
      <Card>
        <EmptyState icon={<FileX className='h-8 w-8 text-slate-500' />} title='No due diligence data recorded for this deal' />
      </Card>
    );
  }
    ...
    <div className='space-y-6'>
      ...
      <Card title='Diligence Workstreams'>
          ...
          <table className='w-full text-sm'>
            <thead>
              <tr className='border-b border-slate-700 bg-slate-700/40'>
                <th className='px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-200'>Workstream</th>
                <th className='px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-200'>Status</th>
                <th className='px-5 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-200'>&nbsp;</th>
              </tr>
            </thead>
            <tbody>
              {diligence.workstreams.map((workstream, i) => (
                <tr key={workstream.name} className={cn(i < diligence.workstreams.length - 1 && 'border-b border-slate-700')}>
                  <td className='px-5 py-3.5 font-medium text-slate-100'>{workstream.name}</td>
        ...
        <div className='mt-4 flex items-center gap-3'>
          <div className='h-1.5 flex-1 overflow-hidden rounded-full bg-slate-700'>
            <div className='h-full rounded-full bg-indigo-500' style={{ width: `${(completeCount / diligence.workstreams.length) * 100}%` }} />
          </div>
      ...
      >
        <form onSubmit={handleAddFinding} className='space-y-4'>
          <div>
            <label className='mb-1 block text-xs font-medium text-slate-200'>Title</label>
            <input type='text' required value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} className={inputClass} />
          </div>

          <div>
            <label className='mb-1 block text-xs font-medium text-slate-200'>Description</label>
            <textarea
              rows={3}
          ...
          <div>
            <label className='mb-1 block text-xs font-medium text-slate-200'>Risk Level</label>
            <select value={form.riskLevel} onChange={(e) => setForm((f) => ({ ...f, riskLevel: e.target.value as RiskLevel }))} className={inputClass}>
          ...
          <div>
            <label className='mb-1 block text-xs font-medium text-slate-200'>Mitigation</label>
            <textarea
              rows={3}
          ...
          <div>
            <label className='mb-1 block text-xs font-medium text-slate-200'>Status</label>
            <select value={form.status} onChange={(e) => setForm((f) => ({ ...f, status: e.target.value as FindingStatus }))} className={inputClass}>
